#!/usr/bin/env node
// Audit-temp only. Patches a COPY of the independent runner in /tmp so the
// name-proxy delta metric uses the frozen national-corpus distinctive_core
// methodology. Does not touch lantivo-vf, R2, manifest, Lovable or Supabase.
...
const replacement = String.raw`function nationalIdentityMetrics(basePois,candPois){
 ...
 for(const opp of OPPORTUNITIES){
  ...
  for(const a of targets){
   ...
   for(const e of extras){
    ...
    // Hard HC gate exactly as requested: populated brand on BOTH sides,
    // differing normalized brand identities, and actual match to the target brand.
    if(ab&&eb&&ab!==eb){hcDen++;if(matchBrand(a.brand,e)){hcFp++;if(hcExamples.length<20)hcExamples.push(\`[${opp}] ${a.brand} => ${e.brand} | ${e.name}\`)}}
    // Name-proxy diagnostic follows national-local-brand-corpus: exercise the
    // distinctive_core variant, not the raw/generic label. A source with no
    // distinctive core is ineligible and contributes no denominator.
    if(!variant) continue;
...
const path = "/tmp/SUPER_CHIVAS_IDENTITY_ONLY_GATE_METHOD_RECHECK.mjs";
