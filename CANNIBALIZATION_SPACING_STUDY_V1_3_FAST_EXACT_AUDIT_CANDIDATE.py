...
# -*- coding: utf-8 -*-
r"""
LANTIVO — CANNIBALIZATION SPACING STUDY V1.3 FAST EXACT
...
SCRIPT_VERSION = "CANNIBALIZATION_SPACING_STUDY_V1_3_FAST_EXACT"
...
OUT_DIR = ROOT / "CANNIBALIZATION_SPACING_STUDY_V1_3"
...
# Runtime provenance pinned to the live VF baseline independently audited by Gris + Claude.
RUNTIME_BASELINE_COMMIT = "9409214fb4d003d1a34d5348e5923113f0ef695d"
DIRECT_COMPETITION_FILTER_BLOB_SHA = "b500eb7fa58bafe615a72cf8ed47643bfb1ab0eb"
BRAND_MATCHING_BLOB_SHA = "33c649441428413909455438fdf01d24ea0a267e"

# The five refreshed official DENUE ZIPs are acquired by the companion downloader.
# V1.3 verifies their recorded SHA256 values before touching the study data.
REFRESH_STATES = ("02", "09", "14", "19", "25")
OFFICIAL_DENUE_BASE_URL = "https://www.inegi.org.mx/contenidos/masiva/denue/"
REFRESH_MANIFEST = ZIP_DIR / "DENUE_2026_05_REFRESH_02_09_14_19_25.json"
...
# Runtime opportunity/category universe for BRAND SEARCH (canibalization), byte-for-byte
# equivalent in category semantics to directCompetitionFilter.ts at RUNTIME_BASELINE_COMMIT.
# OXXO autoservicio is canonically overridden to conveniencia in runtime; accepting both
# source categories here captures the same physical outlets while excluding gasolinera.
TARGETS = {
    "OXXO": {
    ...
    "Starbucks": {
        "categories": {"restaurantes", "fast_food"},
        "anchors": (("starbucks",),),
        "exclusions": (),
...
def verify_refresh_manifest():
    if not REFRESH_MANIFEST.exists():
        die(
            f"falta manifest de refresh oficial: {REFRESH_MANIFEST}. "
            "Ejecuta primero DOWNLOAD_DENUE_2026_05_REFRESH_02_09_14_19_25.py"
        )
    try:
        doc = json.loads(REFRESH_MANIFEST.read_text(encoding="utf-8"))
    except Exception as exc:
        die(f"refresh manifest ilegible: {exc}")
    if doc.get("schema") != "LANTIVO_DENUE_2026_05_REFRESH_MANIFEST_V1":
        die(f"refresh manifest schema inesperado: {doc.get('schema')!r}")
    if doc.get("official_base_url") != OFFICIAL_DENUE_BASE_URL:
        die("refresh manifest no apunta al origen oficial esperado de INEGI")
    states = tuple(doc.get("states") or ())
    if states != REFRESH_STATES:
        die(f"refresh manifest states drift: {states} != {REFRESH_STATES}")
    entries = doc.get("entries") or []
    by_state = {str(e.get("state") or ""): e for e in entries}
    if set(by_state) != set(REFRESH_STATES) or len(entries) != len(REFRESH_STATES):
        die("refresh manifest no contiene exactamente los 5 estados requeridos")
    verified = []
    for state in REFRESH_STATES:
        e = by_state[state]
        filename = f"denue_{state}_csv.zip"
        expected_url = OFFICIAL_DENUE_BASE_URL + filename
        if e.get("filename") != filename or e.get("url") != expected_url:
            die(f"refresh manifest {state}: filename/url drift")
        expected_sha = str(e.get("sha256") or "").lower()
        if not re.fullmatch(r"[0-9a-f]{64}", expected_sha):
            die(f"refresh manifest {state}: SHA256 inválido")
        zp = ZIP_DIR / filename
        if not zp.exists():
            die(f"refresh ZIP faltante después del downloader: {zp}")
        if int(e.get("bytes") or -1) != zp.stat().st_size:
            die(f"refresh ZIP {state}: bytes drift")
        actual_sha = sha256_file(zp)
        if actual_sha != expected_sha:
            die(f"refresh ZIP {state}: SHA256 drift {actual_sha} != {expected_sha}")
        verified.append({
            "state": state, "filename": filename, "url": expected_url,
            "sha256": actual_sha, "bytes": zp.stat().st_size,
            "http_last_modified": e.get("http_last_modified"),
            "http_etag": e.get("http_etag"),
            "main_csv": e.get("main_csv"),
            "rows": e.get("rows"),
        })
    return {
        "manifest": str(REFRESH_MANIFEST),
        "manifest_sha256": sha256_file(REFRESH_MANIFEST),
        "entries": verified,
    }
...
# Normalize the five fixed rule signatures ONCE. The 2.1M-row scan then normalizes
# each establishment name only once, avoiding a needless 5x Unicode-normalization cost.
PREPARED_IDENTITY = {
    brand: {
        "anchors": tuple(tuple(normalize_brand_text(a) for a in sig) for sig in cfg["anchors"]),
        "exclusions": tuple(normalize_brand_text(x) for x in cfg["exclusions"]),
    }
    for brand, cfg in TARGETS.items()
}

def targets_for_report():
    # TARGETS uses sets/tuples for fast deterministic runtime checks; JSON needs lists.
    return {
        brand: {
            "categories": sorted(cfg["categories"]),
            "anchors": [list(sig) for sig in cfg["anchors"]],
...
def match_target_identity_normalized(brand: str, h: str) -> bool:
    if not h:
        return False
    cfg = PREPARED_IDENTITY[brand]
    if any(contains(h, exclusion) for exclusion in cfg["exclusions"]):
        return False
    return any(all(contains(h, a) for a in sig) for sig in cfg["anchors"])

def match_target_identity(brand: str, name: str) -> bool:
    """Python mirror of the five live BRAND_CATALOG rules, evaluated on DENUE name_norm only."""
    return match_target_identity_normalized(brand, normalize_brand_text(name))

def match_target(brand: str, name: str, category: str) -> bool:
    return category in TARGETS[brand]["categories"] and match_target_identity(brand, name)
...
def load_current_universe():
    ...
    outlet_excluded = Counter()
    nominal_by_category = Counter()
    category_excluded = Counter()
    density_grid = Counter()
    t0 = time.time()
    seen_scanned = 0

    def consider(rid, name, lat, lng, cat, source_universe):
        # Deliberately evaluate identity BEFORE the category gate so V1.3 reports
        # every nominal brand hit discarded by category (notably OXXO gasolinera).
        normalized_name = normalize_brand_text(name or "")
        for brand in TARGETS:
            if not match_target_identity_normalized(brand, normalized_name):
                continue
            nominal_by_category[(brand, cat)] += 1
            reason = outlet_exclusion_reason(brand, name or "")
            if reason:
                outlet_excluded[(brand, reason, cat)] += 1
                continue
            if cat not in TARGETS[brand]["categories"]:
                category_excluded[(brand, cat)] += 1
                continue
            target_points[brand].append({
                "id": str(rid), "name": str(name or ""), "lat": lat, "lng": lng,
                "category": cat, "source_universe": source_universe,
            })
    ...
    for i, (rid, name, lat, lng, cat) in enumerate(con.execute(sql), start=1):
        ...
        consider(rid, name, lat, lng, cat, "seen_prod_denue")
        if i % 250_000 == 0:
    ...
    for k in range(0, len(rec_ids), CHUNK):
        ...
        for rid, name, lat, lng in con.execute(
            ...
            lat = float(lat); lng = float(lng)
            if not (-90 <= lat <= 90 and -180 <= lng <= 180):
                die(f"recovered coords inválidas: {rid}")
            density_grid[grid_key(lat, lng, DENSITY_GRID_DEG)] += 1
            consider(rid, name, lat, lng, cat, "c2_recovered")
    if found_recovered != EXPECTED_RECOVERED:
        die(f"recovered lookup drift: {found_recovered:,} != {EXPECTED_RECOVERED:,}")

    con.close()
    audit = {
        "nominal_by_category": nominal_by_category,
        "category_excluded": category_excluded,
        "outlet_excluded": outlet_excluded,
    }
    return target_points, density_grid, audit, {
...
def main():
    t0 = time.time()
    if OUT_DIR.exists():
        die(f"anti-stale: OUT_DIR ya existe; no se reutiliza ni sobreescribe: {OUT_DIR}")
    refresh_provenance = verify_refresh_manifest()
    log("="*100)
    log("LANTIVO — CANNIBALIZATION SPACING STUDY V1.3 FAST EXACT")
    log("DESCRIPTIVE ONLY · NO SCORE POLICY · ZERO PRODUCTION WRITES")
    log("="*100)

    target_points, density_grid, audit, source = load_current_universe()
    ...
    category_audit_rows = []
    for (brand, category), nominal in sorted(audit["nominal_by_category"].items()):
        accepted_cat = category in TARGETS[brand]["categories"]
        category_audit_rows.append({
            "brand": brand,
            "category": category or "<VACIO>",
            "nominal_identity_matches": nominal,
            "runtime_category_accepted": "YES" if accepted_cat else "NO",
            "discarded_by_category": audit["category_excluded"].get((brand, category), 0),
        })
    category_audit_csv = OUT_DIR / f"{SCRIPT_VERSION}_CATEGORY_AUDIT.csv"
    write_csv(
        category_audit_csv, category_audit_rows,
        ["brand","category","nominal_identity_matches","runtime_category_accepted","discarded_by_category"],
    )

    outlet_excluded_rows = [
        {"brand": b, "reason": reason, "category": cat or "<VACIO>", "count": n}
        for (b, reason, cat), n in sorted(audit["outlet_excluded"].items())
    ]
    excluded_csv = OUT_DIR / f"{SCRIPT_VERSION}_EXCLUDED_MATCHES_SUMMARY.csv"
    write_csv(excluded_csv, outlet_excluded_rows, ["brand","reason","category","count"])
    ...
    report = {
        ...
        "source": {
            **source,
            ...
            "direct_competition_filter_blob_sha": DIRECT_COMPETITION_FILTER_BLOB_SHA,
            "brand_matching_blob_sha_provenance": BRAND_MATCHING_BLOB_SHA,
            "refreshed_zip_provenance": refresh_provenance,
        },
        "methodology": {
            ...
            "identity_mirror_status": "LIVE_RULES_EQUAL_ON_NAME_ONLY_POLICY_GATE_STILL_OPEN",
            "identity_mirror_note": (
                "The Python mirror matches normalizeBrandText plus anchors/exclusions for these five live BRAND_CATALOG rules "
                "at the pinned blob. Runtime matchBrand evaluates both poi.brand and poi.name; this DENUE study evaluates name_norm. "
                "Therefore identity equivalence is sufficient for this descriptive run but is NOT certified as full runtime identity parity for policy."
            ),
            "outlet_filter_note": (
                "Study outlet exclusions are explicit and auditable but are not claimed byte-identical to every live runtime commercial filter. "
                "Any policy proposal must close this residual before score/policy authorization."
            ),
        },
        ...
        "category_excluded_match_counts": [
            {"brand": b, "category": cat or "<VACIO>", "count": n}
            for (b, cat), n in sorted(audit["category_excluded"].items())
        ],
        "outlet_excluded_match_counts": outlet_excluded_rows,
    ...
    add("")
    add("CATEGORY AUDIT — NOMINAL BRAND MATCHES OUTSIDE RUNTIME CATEGORY UNIVERSE")
    add("-"*100)
    for r in category_audit_rows:
        if r["runtime_category_accepted"] == "NO" or r["discarded_by_category"]:
            add(
                f"{r['brand']:<24} category={r['category']:<18} nominal={r['nominal_identity_matches']:>7,} "
                f"discarded_by_category={r['discarded_by_category']:>7,}"
            )
    add("")
    add("BRAND SUMMARY")
    add("-"*100)
    ...
    add("- Python identity mirror equals the five live name rules, but runtime also checks poi.brand; full identity parity remains a policy gate.")
    add("- Refreshed ZIPs 02/09/14/19/25 are SHA256-verified against the downloader manifest before the study starts.")
    add("")
    add("NEXT AUDIT:")
    ...
    add(f"CATEGORY AUDIT: {category_audit_csv}")
    summary_path = OUT_DIR / f"{SCRIPT_VERSION}_SUMMARY.txt"
    summary_path.write_text("\n".join(lines) + "\n", encoding="utf-8")

    outputs = [report_path, summary_path, store_csv, brand_csv, density_csv, city_csv, category_audit_csv, excluded_csv]
