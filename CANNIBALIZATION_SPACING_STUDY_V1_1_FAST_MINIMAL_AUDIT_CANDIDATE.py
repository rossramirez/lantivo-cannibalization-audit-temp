...
def compute_neighbors(points):
    ...
    for i, p in enumerate(points):
        ky, kx = grid_key(p["lat"], p["lng"], BRAND_GRID_DEG)
        # ±3 safely covers 3km even at northern Mexico longitudes.
        candidates = set()
        for dy in range(-3, 4):
            for dx in range(-3, 4):
                candidates.update(grid.get((ky+dy, kx+dx), ()))
        dists = []
        for j in candidates:
            if j == i:
                continue
        ...
        row = {
            **p,
            "nearest_m": dists[0] if dists else None,
        }
        for r in RADII_M:
            row[f"n_within_{r}m"] = sum(1 for d in dists if d <= r)
        rows.append(row)
    return rows
